...
char *prog;//Óêàçàòåëü íà òî÷êó âõîäà â ïðîãðàììó
char *p_buf;//Óêàçàòåëü íà òî÷êó ñòàðòà ïðîãðàììû

//int variables[26]={0,0}; //Äîñòóïíî 26 ïåðåñìåííûõ A - Z
double variables[26]={0.0}; //Äîñòóïíî 26 ïåðåñìåííûõ A - Z
	...
	try{
		...
		do{
			...
			if(token_type == VARIABLE){
				...
				putback();//Âîçâðàòèòü ýëåìåíò âî ââîäíîé ïîòîê
...
//	int result;
	...
	do{
		...
		if(token_type == QUOTE){//is string
			...
			putback();
			eval_exp(result);//Òî÷êà âõîäà â ðàçáîð âûðàæåíèÿ
			get_token();
//			printf("X=%lf\n",result);
			std::cout<<result;
//			itoa(result, str, 10);
